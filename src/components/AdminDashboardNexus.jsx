    ...
    <header style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
            <div style={{
                ...
                background: '#f8fafc', borderRadius: 10, border: '1px solid #e5e7eb', width: 280,
            }}>
            ...
            <button style={{
                ...
                border: '1px solid #e5e7eb', background: '#fff', cursor: 'pointer', fontSize: '1.1rem',
            }}>🔔<span style={{ position: 'absolute', top: 6, right: 6, width: 8, height: 8, borderRadius: '50%', background: '#ef4444' }}></span></button>
        </div>
    </header>
);
...
const StatCard = ({ icon, label, value, color, subValue }) => (
    <div style={{
        ...
        boxShadow: '0 1px 3px rgba(0,0,0,0.05)', transition: 'all 0.2s', cursor: 'pointer',
    }}
    ...
    >
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <div>
                <p style={{ margin: '0 0 6px', color: '#64748b', fontSize: '0.85rem' }}>{label}</p>
                <p style={{ margin: 0, fontSize: '1.75rem', fontWeight: 700, color: '#1e293b' }}>{value}</p>
                {subValue && <span style={{ fontSize: '0.8rem', color: '#10b981' }}>{subValue}</span>}
            </div>
            <div style={{
...
const Dashboard = ({ admin, onLogout }) => {
    ...
    const getTitle = () => {
        switch (activeTab) {
            ...
            case 'memories': return 'Memory Manager';
    ...
    return (
        <div style={{ display: 'flex', minHeight: '100vh', background: '#f8fafc', fontFamily: "'Inter', sans-serif" }}>
            ...
            <div style={{ flex: 1, marginLeft: 260, display: 'flex', flexDirection: 'column' }}>
                <Header searchQuery={searchQuery} setSearchQuery={setSearchQuery} title={getTitle()} />
                ...
                <main style={{ padding: '28px 32px', flex: 1 }}>
                    ...
                    {activeTab === 'dashboard' && stats && (
                        <>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 20, marginBottom: 28 }}>
                                <StatCard icon="📷" label="Total Memories" value={stats.totalMemories} color="#6366f1" />
                                <StatCard icon="👥" label="Total Visitors" value={stats.totalVisitors} color="#10b981" subValue={`${stats.todayVisitors} today`} />
                                <StatCard icon="💬" label="Messages" value={stats.totalMessages} color="#f59e0b" subValue={`${stats.unreadMessages} unread`} />
                                <StatCard icon="📈" label="Today Views" value={stats.todayVisitors} color="#ec4899" />
                            </div>

                            {/* Recent Messages */}
                            <div style={{ background: '#fff', borderRadius: 16, padding: 24, boxShadow: '0 1px 3px rgba(0,0,0,0.05)' }}>
                                <h3 style={{ margin: '0 0 20px', color: '#1e293b' }}>💬 Recent Messages</h3>
                                {stats.recentMessages?.length === 0 ? (
                                    <p style={{ color: '#64748b', textAlign: 'center', padding: 40 }}>No messages yet</p>
                                ) : (
                                    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                                        {stats.recentMessages?.slice(0, 4).map((msg, i) => (
                                            <div key={i} style={{ padding: 16, background: msg.isRead ? '#f8fafc' : '#fef3c7', borderRadius: 10, borderLeft: `4px solid ${msg.isRead ? '#e5e7eb' : '#f59e0b'}` }}>
                                                <p style={{ fontWeight: 600, color: '#1e293b', margin: 0 }}>{msg.name}</p>
                                                <p style={{ fontSize: '0.85rem', color: '#64748b', margin: '4px 0 0' }}>{msg.message?.substring(0, 60)}...</p>
                                            </div>
                                        ))}
                                    </div>
                                )}
                    ...
                    {activeTab === 'memories' && (
                        <div style={{ background: '#fff', borderRadius: 16, boxShadow: '0 1px 3px rgba(0,0,0,0.05)', overflow: 'hidden' }}>
                            <div style={{ padding: '20px 24px', borderBottom: '1px solid #f3f4f6', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                <h2 style={{ margin: 0, fontSize: '1.1rem', fontWeight: 600, color: '#1e293b' }}>Memory Inventory ({filteredMemories.length})</h2>
                                ...
                                <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                                    <thead>
                                        <tr style={{ background: '#fafafa' }}>
                                            ...
                                            <th style={{ textAlign: 'left', padding: '14px 20px', fontSize: '0.75rem', fontWeight: 600, color: '#64748b', textTransform: 'uppercase' }}>Title</th>
                                            <th style={{ textAlign: 'left', padding: '14px 20px', fontSize: '0.75rem', fontWeight: 600, color: '#64748b', textTransform: 'uppercase' }}>Description</th>
                                            <th style={{ textAlign: 'left', padding: '14px 20px', fontSize: '0.75rem', fontWeight: 600, color: '#64748b', textTransform: 'uppercase' }}>Date</th>
                                    ...
                                    <tbody>
                                        {filteredMemories.map((memory, idx) => (
                                            <tr key={memory._id} style={{ borderBottom: idx < filteredMemories.length - 1 ? '1px solid #f3f4f6' : 'none' }}
                                                ...
                                                <td style={{ padding: '12px 20px', fontWeight: 500, color: '#1e293b' }}>{memory.title}</td>
                                                <td style={{ padding: '12px 20px', color: '#64748b', fontSize: '0.9rem' }}>{memory.description?.substring(0, 40) || '-'}...</td>
                                                <td style={{ padding: '12px 20px', color: '#64748b', fontSize: '0.85rem' }}>
                                                    {new Date(memory.date || memory.createdAt).toLocaleDateString()}
                                                </td>
                                                <td style={{ padding: '12px 24px' }}>
                                                    <div style={{ display: 'flex', gap: 6 }}>
                                                        <button onClick={() => moveToFirst(memory._id)} title="First" style={{ padding: '6px 10px', borderRadius: 6, border: 'none', background: '#fef3c7', color: '#b45309', fontSize: '0.75rem', cursor: 'pointer' }}>⭐</button>
                                                        <button onClick={() => moveUp(memory._id)} title="Up" style={{ padding: '6px 10px', borderRadius: 6, border: 'none', background: '#dcfce7', color: '#166534', fontSize: '0.75rem', cursor: 'pointer' }}>⬆️</button>
                                                        <button onClick={() => moveDown(memory._id)} title="Down" style={{ padding: '6px 10px', borderRadius: 6, border: 'none', background: '#dbeafe', color: '#1d4ed8', fontSize: '0.75rem', cursor: 'pointer' }}>⬇️</button>
                                                        <button onClick={() => { setEditMemory({ ...memory }); setShowEditModal(true); }} style={{ padding: '6px 10px', borderRadius: 6, border: 'none', background: 'transparent', color: '#6366f1', fontSize: '0.85rem', cursor: 'pointer', fontWeight: 500 }}>Edit</button>
                                                        <button onClick={() => deleteMemory(memory._id)} style={{ padding: '6px 10px', borderRadius: 6, border: 'none', background: 'transparent', color: '#ef4444', fontSize: '0.85rem', cursor: 'pointer', fontWeight: 500 }}>Delete</button>
                                                    </div>
                                                </td>
                                            </tr>
                                        ))}
                    ...
                    {activeTab === 'messages' && (
                        <div style={{ background: '#fff', borderRadius: 16, padding: 24, boxShadow: '0 1px 3px rgba(0,0,0,0.05)' }}>
                            <h2 style={{ margin: '0 0 24px', color: '#1e293b' }}>Messages ({messages.length})</h2>
                            {messages.length === 0 ? (
                                <div style={{ textAlign: 'center', padding: 60, color: '#64748b' }}>
                                    <span style={{ fontSize: 48, display: 'block', marginBottom: 16 }}>💬</span>
                                    <p>No messages yet</p>
                                ...
                                <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                                    {messages.map(msg => (
                                        <div key={msg._id} style={{ padding: 20, background: msg.isRead ? '#f8fafc' : '#fef3c7', borderRadius: 12, border: `2px solid ${msg.isRead ? '#e5e7eb' : '#fcd34d'}` }}>
                                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start' }}>
                                                <div>
                                                    <h4 style={{ margin: '0 0 4px', color: '#1e293b' }}>{!msg.isRead && '🔔 '}{msg.name}</h4>
                                                    <p style={{ margin: '0 0 8px', fontSize: '0.8rem', color: '#64748b' }}>{msg.email} • {new Date(msg.createdAt).toLocaleString()}</p>
                                                    <p style={{ margin: 0, color: '#374151' }}>{msg.message}</p>
                                                </div>
                                                <div style={{ display: 'flex', gap: 8 }}>
                                                    {!msg.isRead && <button onClick={() => markAsRead(msg._id)} style={{ padding: '8px 12px', borderRadius: 8, border: 'none', background: '#22c55e', color: '#fff', fontSize: '0.8rem', cursor: 'pointer' }}>✓ Read</button>}
                                                    <button onClick={() => deleteMessage(msg._id)} style={{ padding: '8px 12px', borderRadius: 8, border: 'none', background: '#ef4444', color: '#fff', fontSize: '0.8rem', cursor: 'pointer' }}>🗑️</button>
                                                </div>
                                            </div>
                                        </div>
                                    ))}
                        ...
                        <div style={{ background: '#fff', borderRadius: 16, padding: 24, boxShadow: '0 1px 3px rgba(0,0,0,0.05)' }}>
                            ...
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: 24 }}>
                                ...
                                <div style={{ padding: 20, background: '#f0fdf4', borderRadius: 12, border: '2px solid #22c55e' }}>
                                    ...
                                    <div>
                                        <label style={{ display: 'block', marginBottom: 4, fontWeight: 600, fontSize: '0.9rem' }}>Photo URL</label>
                                ...
                                <div style={{ padding: 20, background: '#fdf2f8', borderRadius: 12, border: '2px solid #ec4899' }}>
                                    <h3 style={{ margin: '0 0 16px', color: '#be185d' }}>👤 Person 2</h3>
                                    <div style={{ marginBottom: 12 }}>
                                        <label style={{ display: 'block', marginBottom: 4, fontWeight: 600, fontSize: '0.9rem' }}>Name</label>
            ...
            {showAddModal && (
                <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }} onClick={() => setShowAddModal(false)}>
                    <div style={{ background: '#fff', borderRadius: 16, padding: 32, width: '100%', maxWidth: 480 }} onClick={e => e.stopPropagation()}>
                        <h2 style={{ margin: '0 0 24px', color: '#1e293b' }}>Add New Memory</h2>
                        <div style={{ marginBottom: 16 }}>
                            ...
                            <input type="text" value={newMemory.title} onChange={(e) => setNewMemory({ ...newMemory, title: e.target.value })}
                                style={{ width: '100%', padding: 12, borderRadius: 8, border: '2px solid #e5e7eb', boxSizing: 'border-box' }} />
                        </div>
                        <div style={{ marginBottom: 16 }}>
                            ...
                            <textarea value={newMemory.description} onChange={(e) => setNewMemory({ ...newMemory, description: e.target.value })} rows={3}
                                style={{ width: '100%', padding: 12, borderRadius: 8, border: '2px solid #e5e7eb', boxSizing: 'border-box', resize: 'vertical' }} />
                        </div>
                        <div style={{ marginBottom: 20 }}>
                            <label style={{ display: 'block', marginBottom: 6, fontWeight: 600 }}>Image *</label>
                        ...
                        <div style={{ display: 'flex', gap: 12 }}>
                            <button onClick={() => setShowAddModal(false)} style={{ flex: 1, padding: 14, borderRadius: 10, border: '2px solid #e5e7eb', background: '#fff', fontWeight: 600, cursor: 'pointer' }}>Cancel</button>
                            <button onClick={addMemory} disabled={saving} style={{ flex: 1, padding: 14, borderRadius: 10, border: 'none', background: 'linear-gradient(135deg, #6366f1, #8b5cf6)', color: '#fff', fontWeight: 600, cursor: 'pointer', opacity: saving ? 0.7 : 1 }}>
                                {saving ? 'Uploading...' : 'Add Memory'}
                            </button>
                        </div>
                    </div>
                </div>
            ...
            {showEditModal && editMemory && (
                <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }} onClick={() => setShowEditModal(false)}>
                    <div style={{ background: '#fff', borderRadius: 16, padding: 32, width: '100%', maxWidth: 480 }} onClick={e => e.stopPropagation()}>
                        <h2 style={{ margin: '0 0 24px', color: '#1e293b' }}>Edit Memory</h2>
                        {editMemory.imageUrl && <img src={editMemory.imageUrl} alt="" style={{ maxWidth: '100%', maxHeight: 120, borderRadius: 8, marginBottom: 16 }} />}
                        <div style={{ marginBottom: 16 }}>
                            <label style={{ display: 'block', marginBottom: 6, fontWeight: 600 }}>Title *</label>
                        ...
                        <div style={{ display: 'flex', gap: 12 }}>
                            <button onClick={() => setShowEditModal(false)} style={{ flex: 1, padding: 14, borderRadius: 10, border: '2px solid #e5e7eb', background: '#fff', fontWeight: 600, cursor: 'pointer' }}>Cancel</button>
                            <button onClick={updateMemory} disabled={saving} style={{ flex: 1, padding: 14, borderRadius: 10, border: 'none', background: 'linear-gradient(135deg, #6366f1, #8b5cf6)', color: '#fff', fontWeight: 600, cursor: 'pointer', opacity: saving ? 0.7 : 1 }}>
